var subHeader_vue = new Vue({
    ...
    methods: {
        ...
        loadCateItems() {
            ...
            axios.get("/SubHeader/GetAllCategory")
                .then((response) => {
        ...
        async addCategory() {
            try {
                ...
                formData.append('LinkSub', this.LinkNews);
                formData.append('IDMainHeader', this.CateId);

                await axios.post('/SubHeader/AddCategory', formData,
                    {
                        headers: {
        ...
        getItemsById(id) {
            axios.get(`/SubHeader/GetByIDCategory?id=${id}`)
                .then((response) => {
                    this.idCategory = response.data.id;
                    this.NameCategory = response.data.name;
                    this.Icon = response.data.icon;
                    this.LinkNews = response.data.linkSub;
                    this.CateId = response.data.idMainHeader;
                    return Promise.resolve();
                });
        },
        resetDataCategory() {
        ...
        async editCategory() {
            try {
                ...
                await axios.post('/SubHeader/UpdateCategory', formData,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
        ...
        getItemsByIdDelete(id) {
            axios.get(`/SubHeader/GetByIDCategory/${id}`)
                .then((response) => {
                    this.idCategory = response.data.id;
                    if (this.idCategory != null) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const formData = new FormData();
                                formData.append('ID', this.idCategory);
                                axios.post('/SubHeader/DeleteCategory', formData, {
                                    headers: {
